...
const HomeCompleters = () => {
  ...
  useEffect(() => {
    const fetchCompleters = async () => {
      try {
        setLoading(true);
        const res = await axios.get('http://localhost:5000/api/completers');
          ...
          {completers.map((completer) => (
            ...
            >
              ...
              <div className="flex flex-row flex-grow gap-4">
                {/* Left Column: image + (potential) funding */}
                <div className="w-1/2 flex flex-col items-center">
                  {/* Fixed height container (h-42) with absolute-positioned image */}
